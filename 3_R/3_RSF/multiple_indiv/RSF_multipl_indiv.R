...
### 1bis_Creation of a variable fact.visitor.nb ----
#********************************************************************
...
load(file = file.path(base,"Tetralps","3_R","0_Heavy_saved_models","birds_3V", "multipl_telemetry_winter.Rdata"))
load(file = file.path(base,"Tetralps","3_R","0_Heavy_saved_models","birds_3V", "multipl_guess_winter.Rdata"))
load(file = file.path(base,"Tetralps","3_R","0_Heavy_saved_models","birds_3V", "multipl_fit_winter.Rdata"))
load(file = file.path(base,"Tetralps","3_R","0_Heavy_saved_models","birds_3V", "multipl_akde_winter.Rdata"))
...
# Load the outputs of tele_akde with visitor number as continuous variable
load(file = file.path(base,"Tetralps","3_R","0_Heavy_saved_models","birds_3V", "multipl_telemetry_winter_saison_2025_01_16.Rdata"))
load(file = file.path(base,"Tetralps","3_R","0_Heavy_saved_models","birds_3V", "multipl_guess_winter_saison_2025_01_16.Rdata"))
load(file = file.path(base,"Tetralps","3_R","0_Heavy_saved_models","birds_3V", "multipl_fit_winter_saison_2025_01_16.Rdata"))
load(file = file.path(base,"Tetralps","3_R","0_Heavy_saved_models","birds_3V", "multipl_akde_winter_saison_2025_01_16.Rdata"))
...
system.time(
RSF_results_multpl_birds <- RSF_birds(  telemetry_list = l_telemetry_winter, 
...
load(file = file.path(base, "Tetralps", "5_OUTPUTS", "RSF", "rsf.fit_results", paste0("rsf_43birds_individual_2025_01_16.Rdata")))
...
#********************************************************************
# Assuming l_telemetry_winter is the nested list of telemetry objects
...
RSF_results_multpl_birds <- lapply(sum_rsf_multipl, summary)
...
# Identify outliers
# dtaa<- rsf_results_table %>% filter(covariates %in% c("Buildings"))
# is_outlier <- function(x) {
#   x %in% boxplot.stats(x)$out
# }
# is_outlier(dtaa$est)
# dtaa[is_outlier(dtaa$est),"bird"]
...
# ggplot(data = rsf_results_table %>% filter(covariates %in% c("leks")), aes(y = covariates, x = est))+
  ...
  geom_jitter(
    aes(color = ifelse(bird %in% covid, "Monitored during covid", "Others")), # Group names for the legend
    size = 3,
    alpha = 0.6
  ) +
  scale_color_manual(
...
ggplot(data = rsf_results_table %>% filter(covariates %in% c("leks")), aes(y = covariates, x = est))+
  ...
  # geom_jitter(color = alpha("black",0.6))+
  geom_jitter(
    aes(color = ifelse(bird %in% females, "Female", "Male")), # Group names for the legend
    size = 3,
    alpha = 0.6
  ) +
  scale_color_manual(
    name = "Bird groups", # Legend title
    values = c(
      "Female" = "deeppink", # Group "covid_all" in black
      "Male" = "turquoise"        # Others in red
    ),
    labels = c("females" = "Female", "males" = "Male") # Custom labels
  ) +
  # geom_text(
  #   aes(
  #     label = bird,             # Add bird names as labels
...
### 3.7_Analysing the differences between groups
#********************************************************************
# Assign "covid_period" or "normal_period" based on whether `names` is in `covid_all`
rsf_results_table$period <- ifelse(rsf_results_table$bird %in% covid_all, 
                                   "covid_period", 
                                   "normal_period")

ggplot(data = rsf_results_table %>% filter(covariates %in% c("strava", "strava:total.visitors.std", "Cliffs", "Trees", "Shrubs", "Buildings")), 
       aes(y = covariates, x = est))+
  # ggplot(data = rsf_results_table %>% filter(covariates %in% c("leks")), aes(y = covariates, x = est))+
  # geom_boxplot(aes(color = covariates, fill = factor(period)), fill = alpha("grey", 0.2), notch = TRUE)+
  # Boxplots colored by covariates, grouped by period
...
rsf_results_table <- rbind(rsf_results_table, rsf_results_table2)
...
bartlett.test(est~period, data = rsf_results_table[!is.na(rsf_results_table$period) & rsf_results_table$covariates == "strava:total.visitors.std", ]) 
# --> p-value < 2.2e-16 < 0.05 --> variances very different, so test non parametric
# fligner.test(est~period, data = rsf_results_table[!is.na(rsf_results_table$period), ]) # tests d’homogénéité des variances qui est le plus robuste contre les écarts de normalité
# --> p-value = 0.3197 > 0.05 --> variances not necessarily different, so test 

variable = "strava"
t.test(est~period, data = rsf_results_table[!is.na(rsf_results_table$period) & rsf_results_table$covariates == variable, ]) 
wilcox.test(est~period, data = rsf_results_table[!is.na(rsf_results_table$period) & rsf_results_table$covariates == variable, ]) 
