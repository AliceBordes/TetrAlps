...
# 3V human traffic
  # Méribel 
meribel_human_traffic<-read_excel(sheet="Passage Détail",file.path(here(),"1_RAW_DATA/human_traffic_in_ski_resorts/Meribel_mottaret_RM_Historique_des_passages_2017_22/Passage Mensuel par RM Mottaret 22-23.xlsx"))
...
meribel_human_traffic<-meribel_human_traffic %>% replace(is.na(.), 0) # assuming all the NA = 0 visitors
  ...
  # Val Thorens, Les Ménuires
valtho_human_traffic<-read_excel(sheet="2022-23",file.path(here(),"1_RAW_DATA/human_traffic_in_ski_resorts/ValThorens_Orelle_RM_Historique_des_passages_2017_22.xlsx"))

  # Courchevel
courch_human_traffic<-read_excel(sheet="Details",file.path(here(),"1_RAW_DATA/human_traffic_in_ski_resorts/Courchevel_RM_Historique_des_passages_2017_22/Comptages_Passages_2022_2023.xlsx"))
...
#### 1.1_Visualizing osm ski trails in the Trois Vallées ski resort 
...
png(filename = paste0(here(), "/5_OUTPUTS/human_traffic.png"),height = 4200,width = 4600,res=300) # Naming files correctly
ggplot()+
...
#### 1.2_Visualizing osm ski trails overlapping strava ski trails in the Trois Vallées ski resort 

png(filename = paste0(here(), "/5_OUTPUTS/winter_trails.png"),height = 4200,width = 4600,res=300) # Naming files correctly
par(oma = c(1,1,1,1))
plot(strava,ext=e,
...
#### 1.3_Visualizing osm ski cables in the Trois Vallées ski resort 

png(filename = paste0(here(), "/5_OUTPUTS/ski_lifts.png"),height = 3200,width = 4600,res=300) # Naming files correctly
ggplot()+
...
#### 1.4_Visualizing osm ski cables overlapping strava ski cables in the Trois Vallées ski resort 
...
#### 2_Calculate human traffic 


#Méribel
# sum
meribel_human_traffic_sum <- apply(meribel_human_traffic[,3:ncol(meribel_human_traffic)],2,sum) 

# Union of cables_3V_osm_WGS84_sf and meribel_human_traffic_sum dataset
meribel_human_traffic_sum<-data.frame("cable"=names(meribel_human_traffic_sum),"sum_visitors"=meribel_human_traffic_sum,"resort"=rep("Méribel",length(meribel_human_traffic_sum)))
meribel_human_traffic_sum$cable<-stri_trans_general(meribel_human_traffic_sum$cable, id = "Latin-ASCII")  # delete accents

find.list <- list("MO TC ", "MO TK ","MO TSD ")
# in REGEX, | also represents "OR"
find.string <- paste(unlist(find.list), collapse = "|")

meribel_human_traffic_sum$cable <- gsub(find.string,"",meribel_human_traffic_sum$cable) 
...
# bind dataset all 3V 
human_traffic_sum_3V <- bind_rows(meribel_human_traffic_sum,valtho_human_traffic_sum,courch_human_traffic_sum)
...
# Vizualizing the final raster
png(filename = paste0(here(), "/5_OUTPUTS/ski_lift_traffic.png"),height = 4200,width = 4600,res=300) # Naming files correctly
par(oma = c(1,1,1,1))
ggplot()+
