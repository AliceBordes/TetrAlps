...
visu_HR<-function(background=FALSE,season=FALSE,colorby="indiv",color_palette=FALSE,showleks=FALSE,showpoints=FALSE,writeplot=FALSE)
{
  #graph options 
  if(season=="hiver")
  {
  ...
  if(writeplot==TRUE)
  {
    png(filename = paste0(base, "/5_OUTPUTS/RSF/home_range_akde/home_ranges_",
                          ...
                          ".png"),
        height = 4000,width = 4600,res=300) # Naming files correctly
  }
  ...
  if(colorby=="4seasons")
  {
    data_akde_winter<-get(paste0("grouse_winter_akde_saved_hiver_malefemelle"))
    data_akde_spring<-get(paste0("grouse_winter_akde_saved_printemps_malefemelle"))
    data_akde_autumn<-get(paste0("grouse_winter_akde_saved_automne_malefemelle"))
    data_akde_summer<-get(paste0("grouse_winter_akde_saved_ete_malefemelle"))
    
    data_telemetry_winter<-get(paste0("grouse_winter_telemetry_hiver_malefemelle"))
    data_telemetry_spring<-get(paste0("grouse_winter_telemetry_printemps_malefemelle"))
    data_telemetry_autumn<-get(paste0("grouse_winter_telemetry_automne_malefemelle"))
    data_telemetry_summer<-get(paste0("grouse_winter_telemetry_ete_malefemelle"))
  }else{
    
    if(colorby=="sex")
    {
      data_akde_fem<-get(paste0("grouse_winter_akde_saved_",season,"_femelle"))
      data_akde_mal<-get(paste0("grouse_winter_akde_saved_",season,"_male"))
      data_telemetry_fem<-get(paste0("grouse_winter_telemetry_",season,"_femelle"))
      data_telemetry_mal<-get(paste0("grouse_winter_telemetry_",season,"_male"))
    }else{
      data_akde<-get(paste0("grouse_winter_akde_saved_",season,"_malefemelle"))
      data_telemetry<-get(paste0("grouse_winter_telemetry_",season,"_malefemelle"))
    }
  }
    ...
    if(showpoints==TRUE)
    {
      for (i in 1:length(data_telemetry))
      {
        plot(data_telemetry[[i]],UD=data_akde[[i]],
             units=F,xlim=c(e[1],e[2]),ylim=c(e[3],e[4]),add=T,col.grid=NA,bty="n",col.UD=colors[i],col.level=colors[i],level=NA)
      }
  ...
  if(colorby=="sex")
  {
    ...
    if(showpoints==TRUE)
    {
      for (i in 1:length(data_telemetry_mal))
      {
        plot(data_telemetry_mal[[i]],UD=data_akde_mal[[i]],
             units=F,xlim=c(e[1],e[2]),ylim=c(e[3],e[4]),add=T,col.grid=NA,bty="n",col.UD="turquoise")
      }
    
      for (i in 1:length(data_telemetry_fem))
      {
        plot(data_telemetry_fem[[i]],UD=data_akde_fem[[i]],
             units=F,xlim=c(e[1],e[2]),ylim=c(e[3],e[4]),add=T,col.grid=NA,bty="n",col.UD="deeppink")
      }
    }
    if(showpoints==FALSE)
    {
   ...
   if((colorby!="sex" & colorby!="indiv" & colorby!="4seasons")|colorby==FALSE)
    ...
    if(showpoints==TRUE)
    {
      ...
      {
        plot(data_telemetry[[i]],UD=data_akde[[i]],
             units=F,xlim=c(e[1],e[2]),ylim=c(e[3],e[4]),add=T,col.grid=NA,bty="n",col.UD=season_col)
