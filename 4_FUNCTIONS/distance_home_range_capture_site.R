...
#   
#   #function
#   HR_dist_from_centroid<-c()
#   HR_dist_from_clothest<-c()
#   
...
#     poly_95_3<-(poly_95_2@polygons[[paste(data_akde[[i]]@info$identity, "95% est")]])
#     
#     
#     
#     polygon_coords<-list()
...
#     # plot(synth_bg_3V$geometry_lambert[synth_bg_3V$ani_nom==data_akde[[i]]@info$identity],add=T,col="blue")
#     
#     centroid<-st_centroid(multi_sfc)
#     capture_site<-synth_bg_3V$geometry_lambert[synth_bg_3V$ani_nom==data_akde[[i]]@info$identity]
#     
#     # calculate the distance home-range centroid to capture site
#     HR_dist_from_centroid[i]<-st_distance(centroid,capture_site)
#     
#     
#     
#     
...
#       
#       dist_from_clothest[k]<-st_distance(point_sfc,capture_site)
#       
#     }
#     
...
#           # par(oma = c(1,1,1,1))
#           plot(data_akde[[i]],
#                units=F,xlim=c(e[1],e[2]),ylim=c(e[3],e[4]),col.grid=NA,bty="n",col.UD=season_col,
#                col.level=season_col,level=NA,
#                # main=paste("bird:",data_akde[[i]]@info$identity),
#                # sub=paste("\nDistance from centroid =",round(HR_dist_from_centroid[i],0),"m","\nDistance from clothest =",round(HR_dist_from_clothest[i],0),"m"),
...
#           
#           points(synth_bg_3V$x_capture_lambert[synth_bg_3V$ani_nom==data_akde[[i]]@info$identity],synth_bg_3V$y_capture_lambert[synth_bg_3V$ani_nom==data_akde[[i]]@info$identity],col="black",cex=1,type="p",pch=20)
#           mtext(side=3, line=-2.8, adj=0.1, cex=1, paste("\nDistance from centroid =",round(HR_dist_from_centroid[i],0),"m","\nDistance from clothest =",round(HR_dist_from_clothest[i],0),"m"))
#           
#           
...
#   
#   print(paste("Average distance from capture site to centroid of home range:",round(mean(HR_dist_from_centroid),0),"m"))
#   print(paste("Related sd:",round(sd(HR_dist_from_centroid),0),"m"))
#   print(paste("Average distance from capture site to closest point of home range:",round(mean(HR_dist_from_clothest),0),"m"))
#   print(paste("Related sd:",round(sd(HR_dist_from_clothest),0),"m"))
#   
#   return()
# }
