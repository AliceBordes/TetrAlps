...
plot_check_RSF_res_continuous_data<-function(bird_num,raster,analysis_object,season_text,writeplot=FALSE)
{
  ...
  # Graph title options
  if (any(synth_bg_3V$sexe[synth_bg_3V$ani_nom == data_akde[[bird_num]][["hiver1"]]@info$identity] == "male")) {
    sexe_ani <- "male"
    sex_color <- "turquoise3"
  } else if (any(synth_bg_3V$sexe[synth_bg_3V$ani_nom == data_akde[[bird_num]][["hiver1"]]@info$identity] == "femelle")) {
    sexe_ani <- "female"
    sex_color <- "deeppink"
  } else {
    sexe_ani <- "unknown"
  ...
  poly_95_bird<-SpatialPolygonsDataFrame.UD(data_akde[[bird_num]][["hiver1"]],level.UD=0.95,level=0.95)
  crs(poly_95_bird)<-"+init=epsg:2154"
  poly_95_bird<-st_as_sf(poly_95_bird)
  
  #set names to plot home range incertitude
  low<-paste(data_akde[[bird_num]][["hiver1"]]@info$identity,"95% low")
  high<-paste(data_akde[[bird_num]][["hiver1"]]@info$identity,"95% high")
  est<-paste(data_akde[[bird_num]][["hiver1"]]@info$identity,"95% est")
  ...
  if(raster=="habitats"||raster=="habitats_9")
  {
    ...
    if(analysis_object=="HR")
    {
      ...
      g_mask_telemetry<-ggplot()+
        ...
        coord_sf(xlim = c(st_bbox(poly_95_bird)["xmin"], st_bbox(poly_95_bird)["xmax"]),
                 ylim = c(st_bbox(poly_95_bird)["ymin"], st_bbox(poly_95_bird)["ymax"]))+
        geom_point(data=data_telemetry[[bird_num]][["hiver1"]],aes(x=longitude,y=latitude))+
    ...
    data_telemetry[[bird_num]][["hiver1"]]$geometry <- st_sfc(
      lapply(seq_len(nrow(data_telemetry[[bird_num]][["hiver1"]])), function(i) {
        st_point(c(data_telemetry[[bird_num]][["hiver1"]]$longitude[i], 
                   data_telemetry[[bird_num]][["hiver1"]]$latitude[i]))
      }),
      crs = st_crs(poly_95_bird)
    )
    
    # Convert the data frame to an sf object
    telemetry_sf <- st_as_sf(data_telemetry[[bird_num]][["hiver1"]]$geometry)
  ...
  if(raster=="strava")
  {
    ...
    telemetry_values <- telemetry_values[!is.na(telemetry_values)]
    
    # Histogram
    g_histo_telemetry<-ggplot() +
  ...
  g_bird<-grid.arrange(top=textGrob(paste0(str_to_title(data_akde[[bird_num]][["hiver1"]]@info$identity), " (", sexe_ani, ") ", season_text," habitat use"),gp = gpar(fontsize = 18,col=sex_color)),
                       arrangeGrob(g_mask_simple,g_mask_telemetry,ncol=2),arrangeGrob(g_histo_HR_or_study_area,g_histo_telemetry,ncol=2),nrow=2,heights=plot_heights)
  
  
  if(writeplot==TRUE)
  {
    ggsave(path=paste0(base,"/5_OUTPUTS/RSF/check_results/RSF_on_",raster),filename=paste0(data_akde[[bird_num]][["hiver1"]]@info$identity,"_RSF_on_",raster,"_",analysis_object,"_histo.jpg"),plot=g_bird,height =10 ,width =15)
  }
